-- Ian Drake
DELETE FROM `creature` WHERE `id` = 92001;
DELETE FROM `creature` WHERE `id` = 29093;
INSERT INTO `creature` (`id`, `map`, `zoneId`, `areaId`, `spawnMask`, `phaseMask`, `position_x`, `position_y`, `position_z`, `orientation`, `spawntimesecs`, `wander_distance`, `MovementType`, `Comment`)
VALUES (92001, 0, 0, 0, 1, 1, -8807.04, 638.359, 94.312, 3.56047, 180, 0, 0, 'CodeExchange');

-- Ransin Donner
DELETE FROM `creature` WHERE `id` = 92002;
DELETE FROM `creature` WHERE `id` = 2943;
INSERT INTO `creature` (`id`, `map`, `zoneId`, `areaId`, `spawnMask`, `phaseMask`, `position_x`, `position_y`, `position_z`, `orientation`, `spawntimesecs`, `wander_distance`, `MovementType`, `Comment`)
VALUES (92002, 0, 0, 0, 1, 1, -4636.22, -1110, 501.411, 0.628319, 300, 0, 0, 'CodeExchange');

-- Edward Carin
DELETE FROM `creature` WHERE `id` = 92003;
DELETE FROM `creature` WHERE `id` = 29095;
INSERT INTO `creature` (`id`, `map`, `zoneId`, `areaId`, `spawnMask`, `phaseMask`, `position_x`, `position_y`, `position_z`, `orientation`, `spawntimesecs`, `wander_distance`, `MovementType`, `Comment`)
VALUES (92003, 0, 0, 0, 1, 1, 1581.83, 196.686, -43.1022, 2.25663, 180, 0, 0, 'CodeExchange');

-- Zas'Tysh
DELETE FROM `creature` WHERE `id` = 92004;
DELETE FROM `creature` WHERE `id` = 7951;
INSERT INTO `creature` (`id`, `map`, `zoneId`, `areaId`, `spawnMask`, `phaseMask`, `position_x`, `position_y`, `position_z`, `orientation`, `spawntimesecs`, `wander_distance`, `MovementType`, `Comment`)
VALUES (92004, 1, 0, 0, 1, 1, 1991.32, -4652.35, 27.3799, 0.10472, 300, 0, 0, 'CodeExchange');

-- Tharl Stonebleeder
DELETE FROM `creature` WHERE `id` = 92005;
DELETE FROM `creature` WHERE `id` = 16076;
INSERT INTO `creature` (`id`, `map`, `zoneId`, `areaId`, `spawnMask`, `phaseMask`, `position_x`, `position_y`, `position_z`, `orientation`, `spawntimesecs`, `wander_distance`, `MovementType`, `Comment`)
VALUES (92005, 1, 0, 0, 1, 1, 1989.31, -4655.91, 27.7722, 0.15708, 300, 0, 0, 'CodeExchange');

-- Lando Longshot
DELETE FROM `creature` WHERE `id` = 92006;
DELETE FROM `creature` WHERE `id` = 17249;
INSERT INTO `creature` (`id`, `map`, `zoneId`, `areaId`, `spawnMask`, `phaseMask`, `position_x`, `position_y`, `position_z`, `orientation`, `spawntimesecs`, `wander_distance`, `MovementType`, `Comment`)
VALUES (92006, 0, 0, 0, 1, 1, -14397.5, 426.08, 7.97355, 5.51524, 300, 0, 0, 'CodeExchange');
